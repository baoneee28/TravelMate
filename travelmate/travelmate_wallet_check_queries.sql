-- QUICK CHECK QUERIES SAU KHI IMPORT travelmate_wallet_settlement_test_data.sql

-- 1. Tài khoản thử nghiệm
SELECT id, email, role, partner_property_type, bank_name, bank_account_number
FROM users
WHERE email IN ('nguyenhuuan92@gmail.com','contact@dalatpalacehotel.vn','info@rungthongdalat.vn');
...
WHERE b.booking_code LIKE 'BK-PL-%'
ORDER BY b.booking_code;

-- 3. Settlement mới sau khi Admin bấm "Tạo quyết toán tháng trước"
-- Expected cho contact@dalatpalacehotel.vn: gross=2250000, commission=337500, voucher=200000, payout=1712500
...
WHERE u.email = 'contact@dalatpalacehotel.vn'
...
WHERE u.email IN ('contact@dalatpalacehotel.vn','info@rungthongdalat.vn');
...
WHERE wr.request_code LIKE 'WD-PL-%'
...
WHERE (tx.transaction_code LIKE 'WTX-PL-%' OR tx.transaction_code LIKE 'WTX-RT-%')
